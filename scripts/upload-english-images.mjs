...
// Query vocab_images for which lowercase phrase keys already have an image.
...
async function main() {
  ...
  if (!FORCE && plan.length > 0) {
    ...
    const allKeys = [...new Set(plan.map(p => p.phrase.toLowerCase().trim()))];
    ...
    for (let i = plan.length - 1; i >= 0; i--) {
      if (present.has(plan[i].phrase.toLowerCase().trim())) plan.splice(i, 1);
    }
